...
impl Contract {
    ...
    pub fn commit_by_miner(self, request_id: u64, answer: String) {
    ...
    pub fn commit_by_validator(self, request_id: u64, answer: Vec<AccountId>) {
        ...
        match self.get_register_participants(&env::predecessor_account_id()) {
            Some(type_participant) => {
                ...
                if *type_participant == ParticipantType::Validator {
                    let proposal = MinerProposal {
                        //TODO, corregir como hacer el hash del vector con el topten
                        proposal_hash: env::keccak256(answer.as_bytes()),
                        is_revealed: false,
                    };
